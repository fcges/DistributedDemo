...
void UTimerWidget::NativeOnInitialized()
{
	...
	{
		OwningPlayerController->OnTimerUpdated.AddDynamic(this, &UTimerWidget::OnTimeUpdated);
		OwningPlayerController->OnTimerStopped.AddDynamic(this, &UTimerWidget::OnTimeStopped);
	}
...
void UTimerWidget::OnTimeUpdated(float CountdownTimeLeft, ECountdownTimerType Type)
{
...
void UTimerWidget::OnTimeStopped(float CountdownTimeLeft, ECountdownTimerType Type)
{
	if (Type != TimerType) return;
...
{
	TimeSeconds = bCanBeNegative ? TimeSeconds : FMath::Max(0.f, TimeSeconds);
