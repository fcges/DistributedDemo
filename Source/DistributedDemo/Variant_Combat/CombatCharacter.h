...
protected:
	...
	/** Number of combo presses buffered while an attack montage is already playing */
	int32 CachedComboInputCount = 0;
	...
	float ComboInputCacheTimeTolerance = 0.75f;
...
public:
	...
	void ServerDoComboAttackStart();

	UFUNCTION(Server, Reliable)
	void ServerDoChargedAttackStart();

	UFUNCTION(Server, Reliable)
	void ServerDoChargedAttackEnd();

	UFUNCTION(NetMulticast, Reliable)
	void MulticastPlayComboAttack();

	UFUNCTION(NetMulticast, Reliable)
	void MulticastPlayChargedAttack();

	UFUNCTION(NetMulticast, Reliable)
	void MulticastJumpToComboSection(FName SectionName);

	UFUNCTION(NetMulticast, Reliable)
	void MulticastJumpToChargedSection(FName SectionName);
